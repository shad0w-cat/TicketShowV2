    ...
    for bookings in user_shows:
        ...
        if booking.month == month:
            ...
            if (show.name in d.keys()) and (d[show.name]['Booking Date'] == booking.date()):
                ...
                d[show.name]["Booking Date"].append(date_in_words)
                d[show.name]['Venue'].append(venue.name)
...
with app.app_context():
    ...
    def setup_periodic_tasks(sender, **kwargs):
        ...
        sender.add_periodic_task(20.0, daily_task.s(), name='daily')
        sender.add_periodic_task(15.0, monthly_task.s(), name='monthly')
            ...
            try:
                ...
                if send['send'] == 'true':
                    with open("public/send_mail.html","r") as b:
                        ...
                        send_email("simrankukrejaa@gmail.com", subject="Daily Reminder", message=html.render(user = user))
            except:
                pass
    ...
    def monthly_task():
        ...
        for user in all_users:
            ...
            id = user['user_id']
            try:
                d = requests.get(f"http://localhost:8081/get/user_monthly/{id}")
                print(d.json())
                d = d.json()['data'][0]
