...
def daily_task():
    ...
    for user in all_users:
        user_shows = db['user_show'].query.filter(db['user_show'].user_id == user.user_id).all()
    ...
    for user in all_users:
        ...
        for bookings in user_shows:
            ...
            if booking.month() == month:
                show = db.Show.query.filter(db.Show.show_id == bookings.show_id).first()
                venue = db.Venue.query.filter(db.Venue.venue_id == bookings.venue_id).first()
